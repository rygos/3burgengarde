                ...
                <div class="max-w-xl">
                    <section>
                        <header>
                            ...
                            {{ Form::open(['route' => 'calendar.store', 'class' => 'mt-6 space-y-6']) }}
                            <div>
                                <x-input-label for="title" :value="__('Titel')" />
                                <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" required autofocus autocomplete="Titel" />
                                <x-input-error class="mt-2" :messages="$errors->get('title')" />
                            </div>
                            <div>
                                <x-input-label for="start_date" :value="__('Startdatum')" />
                                <x-datetime-local id="start_date" name="start_date" class="mt-1 block w-full" />
                                <x-input-error class="mt-2" :messages="$errors->get('start_date')" />
                            </div>
                            <div>
                                <x-input-label for="description" :value="__('Beschreibung')" />
                                <x-textarea id="description" name="description" type="text" class="mt-1 block w-full" rows="6" autofocus></x-textarea>
                                <x-input-error class="mt-2" :messages="$errors->get('description')" />
                            </div>
                            <div>
                                <x-input-label for="private_description" :value="__('Interne Beschreibung (Nur für Mitglieder Sichtbar!)')" />
                                <x-textarea id="private_description" name="private_description" type="text" class="mt-1 block w-full" rows="6" autofocus></x-textarea>
                            ...
                            <div>
                                ...
                                <x-select name="date_type" id="date_type" class="mt-1 block w-full">
                                    <option value="Alle">KG komplett</option>
                                    <option value="3BG">3Burgengarde</option>
                            ...
                            <div>
                                ...
                                <x-select name="public" id="public" class="mt-1 block w-full">
                                    ...
                                    <option value="0">Nein, nur Intern</option>
                                </x-select>
